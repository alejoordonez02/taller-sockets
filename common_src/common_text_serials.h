...
inline const std::map<std::string, OutputType> SRL_TO_OUTPUT_TYPE = get_inverse_map(OUTPUT_TYPE_TO_SRL);
...
inline const std::map<std::string, WeaponName> SRL_TO_WEAPON_NAME = get_inverse_map(WEAPON_NAME_TO_SRL);
...
inline const std::map<std::string, WeaponType> SRL_TO_WEAPON_TYPE = get_inverse_map(WEAPON_TYPE_TO_SRL);
}  // namespace TextConstant
